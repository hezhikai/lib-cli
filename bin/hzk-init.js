...
function downloadAndGenerate() {
  ...
  return pipeAsyncFunctions(
    ...
    (res) => {
      ...
      let web_url = templates.find(
        (temp) => temp.templateName === res.templateName
      ).templateUrl;
      ...
      download(web_url, local_url, (err) => {
        if (!err) {
          ...
          envFile = envFile
            .split('\n')
            .map((k) => {
              if (k.includes('clientId'))
                if (k.includes('appName'))
                  // k = k.replace(/(?<=(clientId *= *))\S+/, res.clientId);
                  // k = k.replace(/(?<=(appName *= *))\S+/, res.appName);
                  return k;
            })
            .join('\n');
          fs.writeFileSync(local_url + '/.env', envFile, 'utf-8');

          let vueConfig = fs.readFileSync(
            local_url + '/vue.config.js',
            'utf-8'
          );
          fs.writeFileSync(
            local_url + '/vue.config.js',
            vueConfig.replace(
              // /(?<=service_ip: 'http:\/\/10.1.6.154:)\d+/,
              res.proxyPort
            ),
            'utf-8'
          );
